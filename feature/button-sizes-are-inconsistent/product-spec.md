...
Organizer row toggles (**Moved**, **Needs location**) are `size="sm"` in the table on laptop and `size="sm"` plus `min-h-11` only in the mobile card layout — another visible split on the same page.
...
2. **Resume on laptop** — User opens Home on a wide screen; hub buttons are not comically large but still align with session CTAs on the same breakpoint rules as Reconcile.
3. **Organizer on phone** — User toggles **Moved** / **Needs location** and then taps **Declare ready to import**; row actions and the phase gate feel intentionally related (see Decisions for row-toggle scope).
...
- Aligning these with the **existing** touch-target convention already applied on Reconciliation, Compare CTAs, and `ConfirmDialog`.
- Updating [docs/ui-rules.md](../../docs/ui-rules.md) if the written rule needs a explicit “marketing shell” clause after decisions land.
...
- New button component library or redesign of shadcn theme tokens (unless `/design` finds a minimal shared wrapper is the only maintainable fix).
...
- Laptop dense tables may keep `sm` row actions **only if** product confirms row toggles are out of scope (see open questions in Plan chat).

## Decisions (optional)

| Date | Decision |
|------|----------|
| — | *Pending Plan conversation* |
